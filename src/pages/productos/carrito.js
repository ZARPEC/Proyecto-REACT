    ...
    <Box sx={{ flexGrow: 1, p: 3, backgroundColor: theme.palette.background.default }}>
      <Grid container spacing={3}>
        <Grid item xs={12} md={8}>
          ...
          {items.map((item) => (
            <Card key={item.id} sx={{ display: 'flex', mb: 2, borderRadius: 2, boxShadow: 3 }}>
              <CardMedia
                component="img"
              ...
              <CardContent sx={{ flex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  <IconButton onClick={() => actualizarCantidad(item.id, -1)} size="small">
                    <RemoveIcon />
                  </IconButton>
                  ...
                  <IconButton onClick={() => eliminarProducto(item.id)} sx={{ ml: 1 }} color="error">
                    <DeleteIcon />
                  </IconButton>
                </Box>
              </CardContent>
          ...
          <Paper elevation={3} sx={{ p: 3, borderRadius: 2 }}>
            ...
            {items.map((item) => (
              <Box key={item.id} sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                ...
                <Typography variant="body1">Q{(item.precio * item.cantidad).toFixed(2)}</Typography>
              </Box>
            ))}
            <Divider sx={{ my: 2 }} />
